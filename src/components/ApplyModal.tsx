    ...
    try {
      ...
      if (!isDemo) {
        await addDoc(collection(db, 'chat_messages'), {
          ...
          text: t('chat.initial_apply_message', { title: job.title, message }),
          jobId: job.id,
          createdAt: serverTimestamp()
        });
      }
            ...
            <div className="p-8">
              ...
              {success ? (
                ...
                <form onSubmit={handleApply} className="space-y-6">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{t('jobs.your_message')}</label>
                    <textarea
                      required
                      rows={5}
                      value={message}
                      ...
                      placeholder={t('jobs.message_placeholder')}
                    />
                  </div>
                  ...
                  >
                    {loading ? t('common.sending') : t('jobs.send_application')}
                  </button>
                </form>
              )}
            </div>
