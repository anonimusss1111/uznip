...
export default function WorkerDashboard() {
  ...
  const statCards = [
    { label: 'Faol arizalar', value: stats.activeApplications, icon: Clock, color: 'bg-blue-600', shadow: 'shadow-blue-600/20' },
    { label: 'Faol shartnomalar', value: stats.activeContracts, icon: CheckCircle, color: 'bg-emerald-600', shadow: 'shadow-emerald-600/20' },
    { label: 'Bajarilgan ishlar', value: stats.completedJobs, icon: Briefcase, color: 'bg-indigo-600', shadow: 'shadow-indigo-600/20' },
    { label: 'Umumiy daromad', value: `${stats.earnings.toLocaleString()} UZS`, icon: TrendingUp, color: 'bg-amber-600', shadow: 'shadow-amber-600/20' },
  ];
    ...
    <DashboardLayout>
      <div className="space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            <h2 className="text-4xl font-black text-slate-900 tracking-tight">Xush kelibsiz, {profile?.fullName.split(' ')[0]}!</h2>
            <p className="text-slate-500 mt-2 font-medium">Bugungi ish qidirish jarayoni qanday ketyapti?</p>
          </div>
          <Link
            to="/jobs?near=true"
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between px-2">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
                ...
                Sizning hududingizdagi ishlar
              </h3>
              <Link to="/jobs" className="text-xs font-black text-blue-600 uppercase tracking-widest hover:text-blue-700 transition-colors">Barchasini koʻrish</Link>
            </div>
            ...
            <div className="space-y-4">
              {loading ? (
                      ...
                      <div className="flex gap-6">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-5 mt-3 text-xs font-bold text-slate-400">
                            <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-blue-400" /> {job.district}</span>
                            <span className="flex items-center gap-2"><Clock className="w-4 h-4" /> {format(job.createdAt?.toDate?.() || new Date(), 'd MMM', { locale: uz })}</span>
                          </div>
                        </div>
                      </div>
                      <div className="text-left sm:text-right pt-4 sm:pt-0 border-t sm:border-t-0 border-slate-50">
                        <p className="text-2xl font-black text-blue-600 tracking-tight">{job.price.toLocaleString()} UZS</p>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5">{job.workType === 'one-time' ? 'Bir martalik' : 'Doimiy'}</p>
                      </div>
                    </div>
                  </Link>
                ))
                ...
                <div className="bg-white rounded-[40px] p-16 text-center border border-slate-100 shadow-sm">
                  ...
                  <p className="text-slate-500 font-bold text-lg tracking-tight">Hududingizda hozircha ishlar yoʻq.</p>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="bg-white rounded-[40px] border border-slate-100 overflow-hidden shadow-sm">
              {loading ? (
                    ...
                    <div key={app.id} className="p-6 hover:bg-slate-50 transition-all group">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="text-[10px] font-bold text-slate-400">
                          {format(app.createdAt?.toDate?.() || new Date(), 'd MMM', { locale: uz })}
                        </span>
                      </div>
                ...
                <div className="p-16 text-center">
                  <p className="text-sm text-slate-400 font-bold">Siz hali arizalar yubormagansiz.</p>
                </div>
              )}
            ...
            <div className="bg-slate-900 rounded-[40px] p-8 border border-slate-800 shadow-2xl space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Link to="/my-profile" className="p-5 bg-slate-800/50 rounded-2xl border border-slate-700/50 text-center hover:border-blue-500 transition-all group">
                  ...
                  <span className="text-[10px] font-black text-white uppercase tracking-widest">Profil</span>
                </Link>
                <Link to="/chat" className="p-5 bg-slate-800/50 rounded-2xl border border-slate-700/50 text-center hover:border-emerald-500 transition-all group">
                  ...
                  <span className="text-[10px] font-black text-white uppercase tracking-widest">Xabarlar</span>
                </Link>
              </div>
            </div>
          </div>
